...
(defun nemaks/treesit-install-all-languages ()
  "Install all tree-sitter grammars compatible with ABI 14."
  (interactive)
  (let ((failed-langs nil))
        ...
        (condition-case err
            (progn
              ...
              (if (treesit-language-available-p language)
                  (message "✓ %s installed successfully" language)
                (message "⚠ %s installed but not available" language)))
          (error
           (push language failed-langs)
           (message "✗ Failed to install %s: %s" language (error-message-string err))))))
    (if failed-langs
        (message "⚠ Failed to install: %s" failed-langs)
      (message "✓ All grammars installed successfully!"))))

;; ;; Active tree-sitter only for certains modes
...
(use-package treesit-auto
  :disabled t ;; to many issue w/ go & docker (ABI 14)
  :ensure t
  :demand t
  :custom
