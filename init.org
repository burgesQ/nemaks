...
*** DONE strange warning when saving file
...
*** DONE strange warning at start
- [X] ■  Warning (elpaca): Init installer version does not match /home/master/.emacs.d/elpaca/builds/elpaca/doc/installer.el.
- [X] ■  Warning (emacs): elpaca loaded before Elpaca activation
*** DONE LSP
*** DONE claude-el
...
Multiple emacs version may coexist, thanks to [[https://github.com/plexus/chemacs2.git][chemacs2]] -
...
(use-package vertico
  ...
  :config
  (vertico-mode)
  (defun nemaks/minibuffer-override-history-keys ()
    ;; git-commit-mode binds M-p/M-n globally via minor-mode-map-alist, which
    ;; beats vertico-map (local map). Push overrides into the higher-priority
    ;; minor-mode-overriding-map-alist so history navigation wins in minibuffers.
  ...
  (add-hook 'minibuffer-setup-hook #'nemaks/minibuffer-override-history-keys))
#+end_src
...
The [[https://github.com/emacs-lsp/lsp-ui][lsp-ui]] project offers much of the display and
interface to LSP.
...
(use-package pyvenv
  ...
  :diminish
  :config
  ;; Auto-detect .venv/ (uv/virtualenv) on Python file open.
  ;; lsp-pyright-python-path is set buffer-local before LSP connects so
  ;; pyright resolves the venv's site-packages for completion.
  (add-hook 'python-mode-hook
            (lambda ()
              (when-let* ((project-root (locate-dominating-file
                                         default-directory "pyproject.toml"))
                          (venv-path (expand-file-name ".venv" project-root))
                          (python-bin (expand-file-name ".venv/bin/python" project-root)))
                (when (file-exists-p python-bin)
                  (setq-local lsp-pyright-python-path python-bin)
                  (pyvenv-activate venv-path)
                  (message "uv venv activated: %s" venv-path))))))
#+end_src
...
LSP via [[https://github.com/artempyanykh/marksman][marksman]] (~cargo install marksman~).
Live preview via [[https://github.com/nickcoutsos/vmd][vmd]] (~npm install -g vmd~).

Note: ~:ensure nil~ because ~markdown-mode~ is pulled in as a transitive dep of ~lsp-mode~/~lsp-ui~.

#+begin_src emacs-lisp
;; markdown-mode is already queued by lsp-mode/lsp-ui — configure only
(use-package markdown-mode
  ...
  (nemaks/leader-keys
    ...
    "m f" '(lsp-format-buffer :wk "Format buffer")
    "m r" '(lsp-rename :wk "Rename symbol")
    "m v" '(vmd-mode :wk "vmd live preview")
    "m p" '(markdown-preview :wk "Built-in preview")
    "m t" '(markdown-toggle-markup-hiding :wk "Toggle markup")
    "m l" '(markdown-insert-link :wk "Insert link")
    "m h" '(markdown-insert-header-dwim :wk "Insert header")))

;; Live preview in a dedicated window (requires: npm install -g vmd)
